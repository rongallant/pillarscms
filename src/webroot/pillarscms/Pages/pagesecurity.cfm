<!--- 
	Page Security Tag v1.0
	Author - Ron Gallant
	File - pagesecurity.cfm
 --->
<cfif thistag.executionmode eq "start">
	<!--- I make sure the tag has all required attributes --->
	<cfif isDefined('session.userid') and len(trim(session.userid))>
	...
	<cfparam name="attributes.pageid" type="guid" />
	...
	<cfif not isDefined('request.qUser')>
		<cfquery name="request.qUser" datasource="#attributes.dsn#">
			SELECT a.identifier, b.permissions
			FROM #attributes.tableprefix#_apps a
			LEFT OUTER JOIN #attributes.tableprefix#_userapps b
				ON a.id = b.fk_app_id
			LEFT OUTER JOIN #attributes.tableprefix#_users c
				ON b.fk_user_id = c.username
			WHERE c.username = <cfqueryparam value="#attributes.UserID#" cfsqltype="cf_sql_varchar" maxlength="8" />
				AND c.enabled = 1
		</cfquery>
	</cfif>
