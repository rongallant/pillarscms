...
------------------------------------------------------------------------------------------->
	 	<cfcatch type="Any">
			<cftry>
	 		<cfif fusebox.useErrorCatch>
				<cfscript>
				fusebox.cfcatch=duplicate(cfcatch); 
				...
				<cfloop condition="len(FB_.errorPath) GT 0"> <!--- as long as error path has more circuits --->
					<cfif fusebox.rethrowError>
						<cfif StructKeyExists(FB_.reverseCircuitPath, FB_.circuitAlias)>
							...
							<cfcatch type="Any">
								<cfset fusebox.cfcatch=duplicate(cfcatch)><cfset fusebox.rethrowError=TRUE></cfcatch>
							</cftry>
						</cfif>
					<cfelse>
						<cfbreak>
...
<cfcatch type="any">
	...
	<cfif fusebox.useErrorCatch>
	<cfset fusebox.cfcatch=duplicate(cfcatch)>
