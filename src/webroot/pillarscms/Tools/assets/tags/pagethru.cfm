...
<CF_PAGETHRU TOTALRECORDS="integer" CURRENTPAGE="integer" TEMPLATEURL="URL Path"
 ...
 - NEXTGROUPSTR (optional) the hyperlink text of the next group button.  The default is ">>".
	You must set IMAGEPATH="NONE" to use this option.
 - DEFAULTSTYLES (optional) Use the default styles for text. Default value is YES.
...
<cfif attributes.defaultstyles eq "yes">
	<cfsavecontent variable="toHead">
		<style media="all" type="text/css">
			#PGtxt {}
			#PGtxtStrong {
				font-weight:bold;
				color: <cfoutput>#attributes.HiLiteColor#</cfoutput>;
			}
		</style>
	</cfsavecontent>
...
<CFPARAM NAME="Attributes.DefaultStyles" DEFAULT="yes" type="string">
<CFSET ErrorCode = 0>
<CFSET PageStr = "">
...
<CFIF MaxPages GT 1>
	<CFSET PageStr = "<span id=""PGtxt"">Page&nbsp;</span>">
	...
	<CFLOOP INDEX="Count" FROM="#FromPage#" TO="#ToPage#">
		<CFIF Count IS Attributes.CurrentPage>
			<CFSET PageStr = PageStr & " <span id=""PGtxtStrong"">#Count#</span> ">
		<CFELSE>
